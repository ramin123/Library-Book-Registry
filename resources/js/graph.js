...
var Graph =
    ...
    (function($) {
        ...
        Private.setOptions = function(obj) {
            ...
            if ((obj && obj.example === true) || !$.isEmptyObject(obj)) {
                //if example chosen or no options given
            ...
            } else if (obj && obj.addStyle === true) {
                //only add styling
                $.extend(this.obj, Private.design(obj), obj);
            } else if (obj) {
                //if user wants nothing done for them just rely only on the object they give
        ...
        Graph.prototype.getData = function() {
            //save a graph as stringified JSON (can expand later)
            return JSON.stringify(this.obj);
        };
        Graph.prototype.genToFunc = function(str) {
            //turn generic string into function name
            str =
                str === 'area' ? 'linear' : str === 'combo' ? 'bar' : str === 'donut' ? 'pie' : str;
            return 'Graph' + str.charAt(0).toUpperCase() + str.substring(1);
        };
        Graph.prototype.expand = function(obj, thing) {
            //expand JSON into a graph (requires 'type' property of 'obj')
        ...
        Graph.prototype.update = function(obj) {
            //recall script file to update graph with new obj
            obj = obj || {};
            ...
            if (self.grid && !self.noLines) {
                ...
                if (self.xGrid) {
                    for (var i = 0; i < xLines; ++i) {
                        ...
                        xGrid +=
                            ...
                            nxt +
                            '" y1="' +
                            (self.height - self.yOffset - self.padding - weird) +
                            '" y2="' +
                            finalY +
                            '"></line>';
                    }
                ...
                if (self.noLines === false) {
                    xGrid +=
                        ...
                        (self.height - self.yOffset - self.padding - weird) +
                        '" y2="' +
                        (self.height - yLines * self.yDist) +
                        '"></line>';
                    yGrid +=
                        '<line x1="' +
                        self.mainOffset +
                        '" x2="' +
                        ((xLines - 1) * self.xDist + self.mainOffset) +
                        '" y1="' +
                        (self.height - self.yDist) +
                        '" y2="' +
                        (self.height - self.yDist) +
                        '"></line>';
                }
        ...
        Graph.prototype.addLabels = function() {
            ...
            for (var i = 1, len = self.y + 1; i <= len; ++i) {
                ...
                yLabels +=
                    ...
                    x +
                    '" y="' +
                    (self.height - (self.yDist * i - self.padding) - 5) +
                    '">' +
                    digit +
                    '</text>';
            }
        ...
        Graph.prototype.finishGraph = function(xLines, yLines, E, thing) {
            ...
            if ((self.legend && self.pieLegend) || self.type === 'pie')
                E.legend = this.addLegend(thing);
            //COMBINING DYNAMICALLY
            E.points = E.points || '';
            for (var i in E) {
                if (E[i] !== E.points && E[i] !== E.tooltips) {
                    //so we can add last to increase z-index
        ...
        Graph.prototype.handleAppend = function(thing, finish) {
            if (thing === 'update') $('#' + this.obj.id).replaceWith(finish);
            //replace old graph with this one
        ...
        Graph.prototype.addLegend = function() {
            ...
            if (self.multipleDataSets || self.type === 'pie') {
                for (var i = 0, len = self.numPoints; i < len; ++i) {
                    ...
                    //TEXT
                    legend +=
                        '<text style="cursor:default;"class="legend-of-' +
                        i +
                        '"x="' +
                        (x + width + 5) +
                        '"y="' +
                ...
                for (var i = 0, len = self.xLength; i < len; ++i) {
                    ...
                    //TEXT
                    legend +=
                        '<text style="cursor:default;"x="' +
                        1.5 * x +
                        '"y="' +
                        (y + height / 2) +
                        '">' +
                        (self.x[i] + ' : ' + self.points[i]) +
                        '</text>';
                    y += self.yDist + self.padding;
                }
            }
            ...
            if (self.special === 'combo') {
                ...
                legend +=
                    '<rect fill="' +
                    (self.design.averageLineColor || 'green') +
                    '"class="averageLine"x="' +
                    x +
                    '" y="' +
                    y +
    ...
    (function($) {
        var GraphLinear = function(obj) {
            ...
            if (this.obj.interactive) {
                ...
                $(document).on('mouseleave', 'svg circle[id$="point"]', function() {
                    ...
                    $(this).css(
                        'opacity',
                        thiz.obj.style['svg[id="' + thiz.obj.id + '"] circle'].opacity || 0.8
                    );
                });
            }
        };
        ...
        GraphLinear.prototype.buildPoints = function(arr, pts) {
            var inc,
                ...
                num,
                i,
                r = pts ? this.obj.averagePointRadius : this.obj.pointRadius,
                self = this.obj;
            ...
            html =
                ...
                str +
                ' ' +
                (self.multipleDataSets && !pts ? 'point-of-' + arr[0] + ' ' : '') +
                '"cx="' +
                x +
                '" cy="' +
                inc +
            ...
            if (
                self.animations &&
                self.pointAnimation &&
                !(self.multipleDataSets && self.special === 'area')
            )
                html +=
                    '<animate attributeName="cy" from="0" to="' +
                    inc +
                    '" dur="' +
                    (self.animationDuration + i / 7) +
                    's" fill="freeze">';
            html += '</circle>';
            //TOOLTIPS
            //rectangle
            html +=
                '<g><rect class="' +
                (self.multipleDataSets && !pts ? 'rect-of-' + arr[0] + ' ' : '') +
                'SVG-tooltip-box"id="' +
                self.id +
                '-point' +
                str +
                ...
                self.rx +
                '"x="' +
                (x - self.padding * 2 - self.tooltipWidth / 2) +
                '"y="' +
                (inc - self.yDist - self.padding * 2) +
                '"height="' +
                (self.yDist + self.padding / 2) +
                '"width="' +
                (50 + self.tooltipWidth) +
                '"/>';
            //text
            html +=
                '<text class="SVG-tooltip"id="' +
                self.id +
                '-point' +
                str +
                '-tooltip" x="' +
        ...
        GraphLinear.prototype.init = function(thing) {
            ...
            if (area && !self.multipleDataSets)
                E.path = '<g class="area"><path id="' + self.id + '-0-path"d="';
            ...
            if (!self.multipleDataSets) {
                ...
                if (!area) {
                    ...
                    E.path +=
                        'L' +
                        self.xOfPoints[self.xOfPoints.length - 1] +
                        ',' +
                        hmdist +
                        ' "></path>';
                }
            } else {
                //this looks similar to the above but we're now using multi dimensional array and different vars
                ...
                for (var i = 0, len = self.numPoints; i < len; ++i) {
                    ...
                    for (var t = 0, len2 = self.points[i].length; t < len2; ++t) {
                        inc =
                            self.height -
                            (self.points[i][t] + self.scale) * (self.yDist / self.scale);
                    ...
                    if (!area) {
                        ...
                        //PATHS
                        paths.push(
                            '<path id="' + self.id + '-' + i + '-path"class="path-of-' + i + '" d="'
                        );
                        paths[i] += 'M' + self.mxOfPoints[i][0] + ',' + hmdist + ' ';
                        paths[i] += 'L' + self.mxOfPoints[i][0] + ',' + self.myOfPoints[i][0] + ' ';
                        for (var t = 0, len2 = self.points[i].length; t < len2; ++t) {
                            paths[i] +=
                                'L' + self.mxOfPoints[i][t] + ',' + self.myOfPoints[i][t] + ' ';
                        }
                        paths[i] +=
                            'L' +
                            self.mxOfPoints[i][self.mxOfPoints[i].length - 1] +
                            ',' +
                            hmdist +
                            '"></path>';
                    }
            ...
            if (!(self.multipleDataSets && area) && self.animations && self.lineAnimation)
                this.animateLines();
        };
        return GraphLinear;
    })(jQuery);
    ...
    (function($) {
        ...
        var GraphBar = function(obj) {
            ...
            if (this.obj.interactive) {
                ...
                $(document).on('mouseleave', 'svg rect', function() {
                    ...
                    $(this).css(
                        'opacity',
                        thiz.obj.style['svg[id="' + thiz.obj.id + '"] .rect'].opacity || 0.8
                    );
                });
            }
        };
        ...
        Private.getAveragePoints = function(points) {
            ...
            for (var i = 0, len = points[0].length; i < len; ++i) {
                avg = 0;
                for (var t = 0, len2 = points.length; t < len2; ++t) {
                    //this lets us loop array td instead of lr with j
                    avg += points[t][j];
                }
        ...
        GraphBar.prototype.init = function(thing) {
            ...
            if (!self.multipleDataSets) {
                for (var i = 0, len = xLines - 1; i < len; ++i) {
                    ...
                    inc =
                        self.points[i] !== 0
                            ? (self.points[i] + self.scale) * (self.yDist / self.scale) - self.yDist
                            : 2;
                    x = i * self.xDist + self.mainOffset;
                    //bars
                    E.rects +=
                        '<rect class="rect bar"id="' +
                        self.id +
                        '-point-' +
                        i +
                        '" x="' +
                        x +
                        '" y="' +
                        (-self.height + self.yDist + self.mainOffset + self.padding / 2) +
                        '" width="' +
                        self.xDist +
                        '"height="' +
                        inc +
                ...
                for (var i = 0, len = xLines - 1; i < len; ++i) {
                    ...
                    E.tooltips +=
                        ...
                        (x + self.padding / 2 - self.tooltipWidth / 2) +
                        '"y="' +
                        (y - weird - self.yDist - self.padding * 2) +
                        '"height="' +
                        (self.yDist + self.padding / 2) +
                        '"width="' +
                        (self.xDist - self.padding + self.tooltipWidth) +
                        '"/>';
                    //tooltip text
                    E.tooltips +=
                        '<text class="SVG-tooltip"id="' +
                        ...
                        i +
                        '-tooltip" x="' +
                        (x + self.xDist / 2 - self.padding) +
                        '" y="' +
                        (y - weird - self.yDist / 2 - self.padding) +
                        '">' +
                        self.points[i] +
                        '</text>';
                }
                ...
                var loopThroughPoints = function(which) {
                    var j = 0;
                    for (var i = 0; i < max; ++i) {
                        //so we get throguh the length of every array
                        for (var t = 0, len = numPoints; t < len; ++t) {
                            //this lets us loop array td instead of lr with j
                            if (t !== npm1) {
                                //skip over spaces array
                                all = t + j + i * npm1;
                                ref = t + j + i * 2;
                                inc =
                                    self.points[t][j] !== 0
                                        ? (self.points[t][j] + self.scale) *
                                              (self.yDist / self.scale) -
                                          self.yDist
                                        : 2;
                                ...
                                if (which === 0) {
                                    E.rects +=
                                        ...
                                        inc +
                                        '"' +
                                        (self.animations && self.barAnimation
                                            ? '><animate attributeName="height" from="0" to="' +
                                              inc +
                                              '" dur="' +
                                              self.animationDuration +
                                    ...
                                    E.tooltips +=
                                        ...
                                        self.rx +
                                        '"x="' +
                                        (x - self.tooltipWidth / 2) +
                                        '"y="' +
                                        (y - weird - self.yDist - self.padding * 2) +
                                        '"height="' +
                                        (self.yDist + self.padding / 2 + 10) +
                                        '"width="' +
                                        (xDist + self.tooltipWidth) +
                                        '"/>';
                                    //tooltip text
                                    E.tooltips +=
                                        '<text class="SVG-tooltip"id="' +
                ...
                if (self.special === 'combo') {
                    ...
                    for (var i = 0, len = points.length - 1; i < len; ++i) {
                        E.lines +=
                            '<path class="averageLine"fill="none"id="' + self.id + '-0-path"d="';
                        E.lines += 'M' + self.xOfPoints[0] + ',' + self.yOfPoints[0] + ' '; //make sure origin is included
                        for (var i = 1; i < self.xOfPoints.length; ++i) {
                            E.lines += 'L' + self.xOfPoints[i] + ',' + self.yOfPoints[i]; //draw line to next point
                        }
            ...
            if (self.special === 'combo' && self.multipleDataSets && self.animations)
                ln.animateLines();
        };
        return GraphBar;
    })(jQuery);
var GraphTable =
    ...
    (function() {
        ...
        GraphTable.prototype.init = function(thing) {
            ...
            var table =
                '<table class="SVG-table"id="' +
                self.id +
                '" border="1"cellpadding="5"><tr><th>#</th><th>' +
                (self.xName || 'X') +
                '</th>' +
                headers +
                '</tr>';
            ...
            if (!this.obj.multipleDataSets) {
                ...
                for (var i = 0, len = self.numPoints; i < len; ++i) {
                    ...
                    E.pie +=
                        ...
                        self.points[i] +
                        ' (' +
                        Private.percent(self.points[i] / max) +
                        ')"id="' +
                        self.id +
                        '-point-' +
                        i +
                ...
                //tooltip box
                E.tooltips +=
                    '<g><circle class="pie-tooltip"id="' +
                    self.id +
                    '-point-' +
                    i +
                    '-tooltip-rect"style="fill:' +
                    (self.design.pieTooltipFill || '#fff') +
                    ';opacity:' +
                    (self.design.pieTooltipOpacity || 0.5) +
                    '"r="' +
                    (self.design.pieTooltipRadius || 60) +
                    '"cx="' +
                    center +
                    '"cy="' +
                    center +
                    '"height="' +
                    (self.design.donutCenterRadius || center / 2) +
                    '"width="' +
                    (self.design.donutCenterRadius || center / 2) +
                    '"/>';
                ...
                for (var i = 0; i < len; ++i) {
                    ...
                    E.tooltips +=
                        ...
                        i +
                        '-tooltip" x="' +
                        (self.design.pieTooltipTextX || tooltipCenter + 10) +
                        '" y="' +
                        (tooltipCenter + center / 4 + sizing) +
                        '">' +
                        self.points[i] +
                        ' (' +
                        Private.percent(self.points[i] / max) +
                        ')</text>';
                    E.tooltips +=
                        '<text class="SVG-tooltip-title"style="display:none;font-weight:bold;font-size:25px;"id="' +
                        self.id +
                        '-point-' +
                        i +
                        '-tooltip-title" x="' +
                        (self.design.pieTooltipTitleX || tooltipCenter + 20) +
                        '" y="' +
                        (tooltipCenter + center / 8) +
                        '">' +
                        self.dataNames[i] +
                        '</text></g>';
                }
                if (self.special === 'donut')
                    //just stick some color circle in the middle :P
                    E.middle =
                        '<g><circle class="middle"cx="' +
                        center +
                        '" cy="' +
                        center +
                        '" r="' +
                ...
                $('svg path[id^="' + self.id + '"].slice')
                    .eq(2)
                    .mouseover(); //select upper slice automatically
            }
        };
        return GraphPie;
    })(jQuery);
        ...
        function(options) {
            ...
            $(document).on('click', 'button[id^="' + id + '-graphify-button-"]', function() {
                ...
                if (thisHTML.hasOwnProperty(type) && !self.animations) {
                    ...
                    //first time running this graph
                    if (type !== 'area' && type !== 'combo' && type !== 'donut') {
                        self.special = false;
                        graph.to(type, self);
                    } else {
                        //area graphs are a subset of linear graphs, combo of bar graphs...so on
                        self.special = type;
                        switch (type) {
                            case 'area':
