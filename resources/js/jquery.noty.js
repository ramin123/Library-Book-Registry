...
var NotyObject = {
    init: function(options) {
        ...
        else options.themeClassName = this.options.theme;
    ...
    _build: function() {
        // Generating noty bar
        var $bar = $('<div class="noty_bar noty_type_' + this.options.type + '"></div>').attr(
            'id',
            this.options.id
        );
        $bar.append(this.options.template)
            .find('.noty_text')
    ...
    show: function() {
        ...
        if (typeof self.options.animation.open == 'string') {
            ...
            self.$bar
                .show()
                .addClass(self.options.animation.open)
                .one(
    ...
    close: function() {
        ...
        if (typeof self.options.animation.close == 'string') {
            self.$bar
                .addClass(self.options.animation.close)
                .one(
                    'webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend',
                    function() {
    ...
    if (notification.options.custom) {
        if (
            ...
            notification.options.custom
                .find(notification.options.layout.container.selector)
...
$.notyRenderer.createModalFor = function(notification) {
    if ($('.noty_modal').length == 0) {
        var modal = $('<div/>')
            .addClass('noty_modal')
            .addClass(notification.options.theme)
