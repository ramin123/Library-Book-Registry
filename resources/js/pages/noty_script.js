...
notes['warning'] =
    "<strong>Warning!</strong> <br /> Best check yo self, you're not looking too good.";
...
function commit_history() {
    $.getJSON('https://api.github.com/repos/needim/noty/commits?callback=?', function(json) {
        ...
        $.each(json.data, function(i, data) {
            ...
            var $committer = $('<td valign="top" style="white-space: nowrap;" />').html(
                data.commit.committer.name
            );
            var $link = $('<a style="font-weight: bold" />')
                .attr('href', 'https://github.com/needim/noty/commit/' + data.sha)
            ...
            ).html(moment(data.commit.committer.date, 'YYYY-MM-DDTHH:mm:ssZ').fromNow());
